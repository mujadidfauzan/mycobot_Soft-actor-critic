...
class FrankaEnv(MujocoRobotEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initial joint configuration for the panda robot
    ...
    def step(self, action) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        reward, reward_info = self.compute_reward(obs["achieved_goal"], self.goal, info)
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        # ===== GET POSITIONS =====
        object_position = achieved_goal
        achieved_goal = np.asarray(achieved_goal)
        desired_goal = np.asarray(desired_goal)
        ee_position = self.get_ee_position()

        # =====  REACH =====
        d_reach = np.linalg.norm(ee_position - object_position)
        reach_reward = -d_reach

        # ===== GRASP =====
        object_height = object_position[..., 2]
        grasp_threshold = self.initial_object_height + 0.01
        grasp_reward = (object_height > grasp_threshold).astype(np.float32)

        # ===== LIFT =====
        lift_reward = np.maximum(0.0, object_height - self.initial_object_height)

        # ===== PLACE =====
        d_place = np.linalg.norm(object_position - desired_goal, axis=-1)
        place_reward = -d_place
